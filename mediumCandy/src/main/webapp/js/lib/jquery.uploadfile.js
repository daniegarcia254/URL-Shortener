...
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            deletelStr: "Delete",
            doneStr: "Done",
            multiDragErrorStr: "Oops! Multiple File Drag &amp; Drop is not allowed.",
            extErrorStr: "That is not a valid type of file. Only <strong>.csv</strong> files allowed.",
            duplicateErrorStr: "Oops! File already exists.",
            sizeErrorStr: "File size exceeded. Allowed max. size is ",
            uploadErrorStr: "Upload is not allowed.",
            maxFileCountErrorStr: "Can't do that. Maximum allowed files are: ",
        ...
        function setDragDropHandlers(obj, s, ddObj) {
            ...
            ddObj.on('drop', function (e) {
                ...
                if(!s.multiple && files.length > 1) {
                    if(s.showError) showAlert(s.multiDragErrorStr);
                    return;
                }
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for(var i = 0; i < files.length; i++) {
                if(!isFileTypeAllowed(obj, s, files[i].name)) {
                    if(s.showError) showAlert(s.extErrorStr);
                    continue;
                }
                if(!s.allowDuplicates && isFileDuplicate(obj, files[i].name)) {
                    if(s.showError) showAlert(s.duplicateErrorStr);
                    continue;
                }
                if(s.maxFileSize != -1 && files[i].size > s.maxFileSize) {
                    if(s.showError) showAlert(s.sizeErrorStr + getSizeStr(s.maxFileSize));
                    continue;
                }
                if(s.maxFileCount != -1 && obj.selectedFiles >= s.maxFileCount) {
                    if(s.showError) showAlert(s.maxFileCountErrorStr + s.maxFileCount);
                    continue;
                }
                obj.selectedFiles++;
